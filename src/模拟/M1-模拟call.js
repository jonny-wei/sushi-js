 ...
 *
 * 模拟思路：
 * (1) 将函数（this）作为属性绑定到 context 上下文，并传入剩余参数；
 * (2) 立即执行这个绑定的函数；
 * (3) 执行完毕后删除这个属性；
 *
 * 原理就是将函数作为传入的上下文参数（context）的临时属性，核心是改变其作用域，
...
Function.prototype.mycall2 = function (thisArg, ...args) {
  ...
  const fn = Symbol("fn");
  context[fn] = this; // 获取调用 call 的函数，将其赋值给 context 的一个属性
...
const result1 = sayHelloTo.mycall1(person, person.name);
const result2 = sayHelloTo.mycall2(person, person.name);
console.log(result1); // 输出：Hello Kimi!
console.log(result2); // 输出：Hello Kimi!
