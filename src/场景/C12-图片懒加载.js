 ...
 * 获取scrollTop的值和getBoundingClientRect方法都会导致回流
...
let lazyLoad = function () {
  let observer = new IntersectionObserver(entries => {
    // 批量处理需要加载的图片
    let imagesToLoad = entries.filter(entry => entry.intersectionRatio > 0).map(entry => entry.target);
    ...
    if (imagesToLoad.length > 0) {
      imagesToLoad.forEach(img => {
        try {
          ...
          img.onerror = () => {
            // 图片加载失败后的回退处理
            img.src = 'fallback-image.png'; // 替换为默认图片
            observer.unobserve(img);
          };
        } catch (error) {
  ...
  // 初始检查，防止页面一开始就需要加载图片
  lazyLoadInitialCheck(observer, imgList);
};

// 初始检查函数
function lazyLoadInitialCheck(observer, imgList) {
  imgList.forEach(img => {
